...
void QCPAxis::placeTickLabel(QCPPainter *painter, double position, int distanceToAxis, const QString &text, QSize *tickLabelsSize)
{
  ...
  //painter->drawRect(finalPosition.x()+labelData.rotatedTotalBounds.left(), finalPosition.y()+labelData.rotatedTotalBounds.top(), labelData.rotatedTotalBounds.width(), labelData.rotatedTotalBounds.height());
  if (orientation() == Qt::Horizontal)
  {
